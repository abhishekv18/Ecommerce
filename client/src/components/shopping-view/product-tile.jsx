  ...
  const { cartItems } = useSelector((state) => state.shoppingCart);
    return ( 
        <Card className='w-full max-w-sm mx-auto '>
          ...
          <img
          src={product?.image}
          alt={product?.title}// releative means neeche and absolute means uper jo move ho sake
...
product?.totalStock === 0 ? (
 ...
 <div className="flex justify-between items-center mb-2">
    <span className={`${
           ...
           <CardFooter>
            {
              product?.totalStock<=0? <Button className='w-full cursor-not-allowed opacity-60' >Out Of Stock</Button>: <Button className='w-full' onClick={()=>handleAddToCart(product?._id,)}>Add to cart</Button>
            }
