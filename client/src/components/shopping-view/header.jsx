...
function MenuItems(){
  ...
  function handleNavigate(getCurrentMenuItem) {// set item remove item
...
function HeaderRightContent(){
 ...
 const [openCartSheet, setOpenCartSheet] = useState(false);
  ...
  return <div className="flex lg:items-center lg:flex-row flex-col gap-4">
    ...
    <UserCartWrapper setOpenCartSheet={setOpenCartSheet} cartItems={ cartItems && cartItems.items && cartItems.items.length > 0
              ? cartItems.items
              : [] }></UserCartWrapper>
       ...
       <Sheet open={open}  onOpenChange={setOpen}>
        <SheetTrigger asChild>
          <Button variant='outline' size='icon' className='lg:hidden'>
            <Menu/>
            <span className="sr-only">Toggle here</span>
          </Button>
        </SheetTrigger>
